...
int son_start() {
	...
	if (sockfd < 0) {
		perror("Client: Creating socket wrong !!\n");
		return -1;
	}
	
	if (connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0) {
		perror("Client: Connecting to the server wrong !!\n");
		return -1;
	}

	printf("In son_start(),, sockfd is %d\n", sockfd);
	return sockfd;
}
...
int main() {
	...
	FILE *f;
	f = fopen("/home/b101220019/client/Oliver Twist.txt","r");
	assert(f!=NULL);
	fseek(f,0,SEEK_END);
	int fileLen = ftell(f);
	fseek(f,0,SEEK_SET);
